...
namespace scanner{
   ...
   namespace{
      ...
      std::string read_numerical_literal(scanner::Scanner &scn){
         ...
         if(first == '0'){   
            ...
            case 'B':{
               ...
               prefix = get_char(scn); 
               cond = [](char c) -> bool{return c == '0' || c == '1';};  //is binary digit?
            ...
            case 'O':{
               ...
               get_char(scn); //advance the 0
               prefix = get_char(scn);
               cond = [](char c) -> bool{return c >='0' && c <= '7';};   //is octal digit?
               reading_decimal = false;
               break;
            }
            case 'x':
            case 'X':{
               ...
               get_char(scn); //advance the 0
               prefix = get_char(scn);
               cond = [](char c) -> bool{return is_decimal_digit(c) || (c>='a'&&c<='f') || (c>='A' && c <= 'F');}; //is hexa digit?
               reading_decimal = false;
               break;
            }
            }
      ...
      std::string read_string_literal(scanner::Scanner &scn){
         std::string ret;
         get_char(scn); //advance "
         ret = read_while(scn, [](char c)->bool{return c!= '\"' && c != '\n' && c != EOF;});
         get_char(scn); //advance "
         return ret;
      }
      ...
      void read_next_token(scanner::Scanner &scn){
         ...
         if(first == EOF){
            ...
            tok.val = read_while(scn, [](char c)->bool{return is_letter(c) || is_decimal_digit(c) || c=='_';});
